...
func isTerminal(file *os.File) bool {
	...
	return terminalIoctl(file.Fd(), syscall.TCGETS, unsafe.Pointer(&state)) == nil
}

func makeRaw(file *os.File) (*terminalState, error) {
	var state terminalState
	if err := terminalIoctl(file.Fd(), syscall.TCGETS, unsafe.Pointer(&state)); err != nil {
		return nil, err
	}
	raw := state
	...
	if err := terminalIoctl(file.Fd(), syscall.TCSETS, unsafe.Pointer(&raw)); err != nil {
		return nil, err
	}
	return &state, nil
}
...
func restoreTerminal(file *os.File, state *terminalState) {
	if state != nil {
		_ = terminalIoctl(file.Fd(), syscall.TCSETS, unsafe.Pointer(state))
	}
}

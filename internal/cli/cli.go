...
func runNotificationsMonitor(ctx context.Context, c *client.Client, formatter *notificationFormatter, out io.Writer, limit int, interval time.Duration, debugLog *notificationDebugLogger) error {
	...
	refreshes := make(chan refreshResult, 1)
	...
	for {
		...
		case refresh := <-refreshes:
			refreshInFlight = false
			checkedAt, result, err := refresh.checkedAt, refresh.result, refresh.err
			if err != nil {
...
func formatTargetStats(kind string, data map[string]any) string {
	...
	case "answer":
		...
		appendFirstNonZeroCount(&stats, "喜欢", data["thanks_count"])
	case "article":
		appendFirstCount(&stats, "赞同", data["voteup_count"])
	...
	case "pin":
		...
		appendFirstNonZeroCount(&stats, "喜欢", reactionStats["like_count"])
		appendFirstNonZeroCount(&stats, "收藏", data["favorite_count"], data["favlists_count"])
	}
...
func appendFirstNonZeroCount(stats *[]string, label string, values ...any) {
	value, ok := firstPresentAny(values...)
	if !ok || toString(value) == "0" {
		return
	}
	*stats = append(*stats, label+" "+display.FormatCount(value))
}

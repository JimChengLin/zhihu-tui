...
- `j` / `k` 或方向键：只滚动正文，到达边缘后停止
- `Ctrl-F` / `Ctrl-B`（兼容 `space` / `b`）：向下 / 向上翻 7/8 页，并标记上一屏的续读行；正文到达边缘后再按一次才切换动态，评论区不会切换动态
- `Ctrl-D` / `Ctrl-U`（兼容 `d` / `u`）：向下 / 向上半页并保留蓝色续读焦点
- `Ctrl-E` / `Ctrl-Y`：向下 / 向上滚动一行
- `n` / `p`、`h` / `l` 或左右方向键：直接切换动态
- `g` / `G`：跳到第一条 / 最后一条已加载动态
- `c`：在当前动态的正文与评论区之间切换
- `v`：正文中赞同 / 取消赞同当前回答；评论区赞同 / 取消赞同蓝色焦点评论
- `z`：切换隐藏动态列表的专注阅读模式
- `o`：用默认浏览器打开当前动态
- `r`：刷新关注流；新内容标题显示为绿色，并标记本次进程内最早 / 最晚展示位置
- `?`：查看完整帮助
- `q` 或 `Ctrl-C`：退出
